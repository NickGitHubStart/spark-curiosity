...
Ersetze Eintraege oder komprimiere sie nur bei echter Redundanz. **Short-Term** kann zusätzlich periodisch vom Companion per LLM aufgeraeumt werden (Hintergrund, grob alle ~200 EVENT_DECISIONs); sonst verdichtest du manuell per `update`/`remove`. **Mid-/Long-Term:** wie oben mit `update` und Zusammenfuehren.
